...
+ 修改日志模拟器配置文件：(hadoop102和hadoop103)
  - 方式一：运行脚本：`generate_one_day_logs.sh 2025-06-04`(修改日志采集日期)
  - 方式二：
...
+ 执行日志生成脚本
  - `ad_mock.sh`
  - 观察HDFS:/Utilities/Browse the file system/origin_data/ad/log/ad_log/XXX文件是否生成
  - 复制以下网址到浏览器去观察HDFS采集情况：http://hadoop102:9870/explorer.html#/
  - 
...
+ 关闭flume、kafka：
  ...
  - 注意当在HBSE中查看没有采集到数据时，则进行“日志数据采集失败”的相关操作即：
  - 日志数据采集失败：
  - 大招：假设前面数据已经完全采集，
  ...
  -  hadoop104的data/bXXX、checkpoint/bXXX目录，重新启动flume采集
    ...
    -  `mv behavior1 behavior1.bak`
    -  当出现以下问题时：
    -  mv: 无法将"behavior1" 移动至"behavior1.bak/behavior1": 文件已存在
    -  可将behavior1.bak改为：behavior1.bak1（后续依次即可）
    -  处理后再执行前面采集数据操作即可。
 
  
 -  若还是采集不到日志数据，则检查一下clickhouse状态情况，然后启动：
    -  查看状态：`sudo systemctl status clickhouse-server`
    -  启动：`sudo systemctl start clickhouse-server`
    -  处理后再执行前面采集数据操作即可。
...
+ 方式一（本质是依次调用方式二的四个脚本,**注意日期与数据日期保持一致**）
  - cd /opt/module/spark/
  - 执行脚本：`load_one_day_data.sh 2025-06-03` （与上面的采集日期要相同）
  - 执行脚本中断，手动执行未执行的脚本及将hive中的数据导入到ClickHouse，**注意日期换为数据日期**。从命令行输出判断执行到了方式二的那个脚本。
...
+ 日志数据采集失败：
  ...
  -  hadoop104的data/bXXX、checkpoint/bXXX目录，重新启动flume采集
    ...
    -  当出现以下问题时：
    -  mv: 无法将"behavior1" 移动至"behavior1.bak/behavior1": 文件已存在
    -  可将behavior1.bak改为：behavior1.bak1即可
